...
export default function Pookalam() {
  ...
  useEffect(() => {
    ...
    const drawPookalam = () => {
      const size = Math.min(window.innerWidth, window.innerHeight) * 0.7; // 70% of screen
      canvas.width = size;
      canvas.height = size;
    ...
    return () => window.removeEventListener("resize", drawPookalam);
  }, []);
  ...
  >
    ...
    <canvas
      ...
      style={{
        display: "block",
        maxWidth: "90vw", // don’t exceed screen
        maxHeight: "90vw",
